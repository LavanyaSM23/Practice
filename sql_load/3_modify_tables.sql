...
COPY company_dim
FROM 'C:\Practice\csv_files\company_dim.csv'
WITH (FORMAT csv, HEADER true, DELIMITER ',', ENCODING 'UTF8');

COPY skills_dim
FROM 'C:\Practice\csv_files\skills_dim.csv'
WITH (FORMAT csv, HEADER true, DELIMITER ',', ENCODING 'UTF8');

COPY job_postings_fact
FROM 'C:\Practice\csv_files\job_postings_fact.csv'
WITH (FORMAT csv, HEADER true, DELIMITER ',', ENCODING 'UTF8');

COPY skills_job_dim
FROM 'C:\Practice\csv_files\skills_job_dim.csv'
WITH (FORMAT csv, HEADER true, DELIMITER ',', ENCODING 'UTF8');
...
AND EXTRACT(MONTH FROM job_postings_fact.job_posted_date) IN (4,5,6);

-- SELECT * FROM job_postings_fact LIMIT 5;

-- SELECT COUNT(job_id) AS job_postings,
-- MAX(job_posted_date) AT TIME ZONE 'UTC' AS date,
